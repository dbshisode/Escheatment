 ...
 --%>   
        ...
        $(document).ready(function() {    
        	...
        	var table = $('#wqTable').DataTable( {
				"ajax": {"url":"get-review-acct-table","dataSrc":""},
				"deferRender": true,         
				"defaultContent": '',
	        	"order": [[ 4, "desc" ]], //trust number
	        	"language": {
	        	      "emptyTable": "No trusts currently under review"
	        	    },       	    
	        	"columns": [
	        		{"orderable": false, data: 'trustId', render: function (data,type,row,meta) {return '<input type="checkbox" class="cbox" name="trustIds" value="' + data + '" />' }},
	       			...
	       			{"orderable": true, data: 'ownerName', render: function (data,type,row,meta) {return '<div class="nameAddress">' + data + row.ownerAddress + '</div><div class="firstLawfulOnwerButtons"><a href="#" role="button" onclick="sendAddlInfo(' + row.trustId + ', ' + row.lawfulOwnerId + ');"><button type="button" class="btn btn-default">Additional Info</button></a>&nbsp;&nbsp;<a href="#" role="button" onclick="sentForReview(' + row.trustId + ', ' + row.lawfulOwnerId + ');"><button type="button" class="btn btn-default ' + row.underReview + '">Sent for Review</button></a></div><div style="clear:both;"></div><div class="addlNames"></div><div class="trustId">' + row.trustId + '</div>'} },
		  ...
		  $('#wqTable').on('click', '#checkAll', function() {
				...
				$.ajax({
					  ...
					  success: function (data) {
		            	  $.each( data, function( key, val ) {
		            		  ...
		            		  addlNames += '<div class="addlLawfulOnwerButtons"><button type="button" class="btn btn-default">Additional Info</button>&nbsp;&nbsp;<a href="#" onclick="sentForReview(' + trustId + ', ' + val.id + ');"><button type="button" class="btn btn-default ' + val.underReview + '">Sent for Review</button></a></div><div style="clear:both;"></div></div>';
		            	  });
		...
		function sentForReview(trustId,lawfulOwnerId) {
			...
			function publication() {
				if (confirm('Mark selected items for publication?')) {		
					
					var checkedTotal = $("input[name=trustIds]:checked").length;	
					
					if (checkedTotal == 0) {
						$('#messageBox').addClass("alert alert-warning");						
						$('#messageText').html("Please select at least one item");
						$('#messageBox').show();
						$('#messageBox').fadeIn().delay(2000).fadeOut();											
					}
					
					$.ajax({
						url : "ready-for-publication",
						type : 'POST',
						//contentType: 'application/json',
						data : $(
								'#reviewForm')
						...
						success : function(
							...
							if (responseJson.status == "SUCCESS") {
								...
								//decrement count in pill nav
								var acctReviewCount = $('#acctReviewCount').text() - checkedTotal;
								$("#acctReviewCount").text(acctReviewCount);
								
								//increment count in pill nav
								var acctPendingCount = $('#acctPendingCount').text();
								$("#acctPendingCount").text(parseInt(acctPendingCount) + parseInt(checkedTotal));
